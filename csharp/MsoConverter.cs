...
namespace MsoRewConverter
{
    ...
    public class ConversionOptions
    {
        ...
        /// <summary>
        /// Filter types to exclude from conversion
        /// </summary>
        public List<string> ExcludedTypes { get; set; } = new List<string> { "Gain Block", "Delay Block" };
    ...
    public class MsoConverter
    {
        ...
        public ConversionResult Convert(string msoContent)
        {
            ...
            try
            {
                ...
                {
                    result.ProcessingLog.Add($"Excluded filter types: {string.Join(", ", _options.ExcludedTypes)}");
                }
        ...
        private List<FilterData> ParseFiltersFromText(string text)
        {
            ...
            foreach (var block in filterBlocks)
            {
                ...
                foreach (var excludedType in _options.ExcludedTypes)
                {
                    if (filterType.Contains(excludedType, StringComparison.OrdinalIgnoreCase))
                    {
                        shouldInclude = false;
                        break;
                    }
